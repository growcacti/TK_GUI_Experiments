...
canvas = tk.Canvas(root, height=800, width=1800, bg="lavender")
canvas.grid(row=1, column=1, rowspan=10, columnspan=20)
# There are 20 buttons that can be used to call added functions
# The Button Bar
def open_file_chooser():
    filename = fildialog.askopenfilename()
    tk.messagebox.askokcancel("You have selected : %s" % filename)
    print("You have selected : %s" % filename)


def open_path_chooser():
    path = fildialog.askdirectory()
    tk.messagebox.askokcancel("You have selected : %s" % filename)
    print("You have selected : %s" % filename)
...
def runpy():
    ...
    x = lb.curselection()[0]
    file = lb.get(x)
    py = subprocess.run(["python3 -m", file])
...
def open_file(event=None):
    ...
    input_file_name = tk.fildialog.askopenfilename(
...
def save_as_file(event=None):
    ...
    input_file_name = tk.fildialog.asksaveasfilename(
        defaultextension=".txt",
        filetypes=[("All Files", "*.*"), ("Text Documents", "*.txt")],
    )
...
shortcutbar = Frame(canvas, height=25)
...
# cmd = eval(f)
cmd = com
btn1 = Button(shortcutbar, text="RUN", command=runpy)
btn1.pack(side=LEFT)
btn2 = Button(shortcutbar, text="Filename VAR", command=open_file_chooser)
btn2.pack(side=LEFT)
btn3 = Button(shortcutbar, text="Scandir", command=os_scandir)
btn3.pack(side=LEFT)
btn4 = Button(shortcutbar, text="Path Choose", command=open_path_chooser)
btn4.pack(side=LEFT)
...
text = Text(canvas, wrap="word", undo=1, bg="cyan")
...
shortcut_bar = Frame(canvas, height=25, background="light green")
